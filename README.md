...
| 命令 | 作用 |
| --- | --- |
| `/refresh` | 触发所有规则；若存在名为 `all` 的全量规则则只触发它 |
...
> [!WARNING]
> **全量规则 `all` 约定**：如果 LitePan 后台存在一条规则名（不区分大小写）为 `all` 的 Webhook 规则，它会被视为“全量规则”。此时 `/refresh`（不带参数）**只触发这条 `all` 规则**，不再逐个触发其他规则；其他规则请用 `/refresh_<规则>` 或 `/refresh <盘名>` 单独触发。若后台没有名为 `all` 的规则，`/refresh` 保持“触发所有规则”的原有行为。
>
> 适用场景：不是每个 STRM 任务都建了联动规则时，用一条 `all` 规则兜底做全量刷新（刷新目录 + 生成所有 STRM 任务），避免 `/refresh` 把 `all` 和各单盘规则重复跑一遍。
